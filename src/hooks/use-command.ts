...
export const useCommand = (
  ...
  const parseAliases = useCallback((fs: Directory) => {
    ...
    const bashrcNode = getNodeFromPath('/home/.bashrc', fs);
  ...
  const executeFile = useCallback((fileNode: FilesystemNode): CommandResult => {
    ...
    }, [aliases, cwd, executeCommandsSequentially, executeFile, isMobile, isRoot, resolvePath, saveFile, setAwaitingConfirmation, toast, updateWarlockAwareness, user, userFilesystem, viewedUser, fetchUserFilesystem, getParentNodeFromPath, getNodeFromPath, handleAdminCommands, handleCtfCommands, handleFileSystemCommands]);
