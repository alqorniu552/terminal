...
export const useCommand = (user: User | null | undefined) => {
  ...
  const getInitialPrompt = useCallback(() => {
    if (user) {
        ...
        if (cwd === userHome) {
            ...
            displayPath = `~${cwd.substring(userHome.length)}`;
        }
        return `${user.email?.split('@')[0]}@command-center:${displayPath}$`;
    }
  ...
  useEffect(() => {
    if (user) {
        ...
        const homeNode = getNodeFromPath('/home');
        if(homeNode && homeNode.type === 'directory' && !homeNode.children[user.email!.split('@')[0]]) {
            homeNode.children[user.email!.split('@')[0]] = {
                type: 'directory',
                children: {
  ...
  }, [cwd, toast, user, getInitialPrompt]);

  return { prompt, setPrompt, processCommand, getWelcomeMessage };
};
